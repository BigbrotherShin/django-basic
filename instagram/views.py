...
def post_like(request, pk):
    ...
    messages.success(request, f"{post}를 좋아요 했습니다.")
...
def post_unlike(request, pk):
    ...
    messages.success(request, f"{post} 좋아요를 취소 했습니다.")
    redirect_url = request.META.get('HTTP_REFERER', 'root')
    return redirect(redirect_url)
...
def user_page(request, username):
    ...
    if request.user.is_authenticated:
        is_follow = request.user.following_set.filter(
            pk=page_user.pk).exists()  # User 객체, AnonymousUser
    else:
        is_follow = False
